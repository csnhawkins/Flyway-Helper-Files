...
A Flyway MCP server is configured in `.mcp.json` - you have direct tools available to inspect,
capture, and generate changes for this database. Read this file before doing any Flyway-related work.
...
- **You may create a branch and commit on my behalf in an interactive session, but confirm with me
  before each commit and before each branch creation** - don't batch several commits behind one
  confirmation. **Never push, and never open a pull request yourself**, regardless of session type -
  that stays a human action. This keeps a clean audit boundary between "what the AI generated/
  committed locally" and "what a human decided was ready to share."
- **Never edit a migration script by hand after it's been generated.** If something's wrong with a
  generated script, regenerate it from an updated schema model rather than patching the SQL directly -
  keeps the schema model and the scripts as the source of truth in sync.
...
- Treat this database and project as sensitive. Don't share schema details, data, or generated SQL
  outside of this session/conversation.
...
## When running headlessly (CI/CD)

If you're being run non-interactively as part of a pipeline, you'll be told explicitly in the
prompt what's expected of you (usually: capture changes, generate a script, review it, and report
a structured result). The boundaries above still apply in full - the pipeline, not you, handles
branching, committing, and raising the pull request.